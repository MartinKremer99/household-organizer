...
describe("barcode UI source", () => {
  it("does not import Dexie, repositories, Supabase, sync, or inventory mutations", () => {
    ...
    expect(sources).not.toMatch(/mutate-inventory/);
    expect(sources).not.toMatch(/addInventory/);
  });
});
